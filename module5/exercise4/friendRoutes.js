...
router.get("/filter", (req, res) => {
  ...
  if (matchingFriends.length > 0) {
    ...
    res.status(404).json({ error: "No friends matching query" });
  }
});
...
router.get("/:id", (req, res) => {
  let friendId = parseIntI(req.params.id, 10); // 'id' here will be a value matching anything after the / in the request path
...
router.post("/", (req, res) => {
  ...
  if (!newFriend.name || !newFriend.gender) {
    ...
    newFriend.id = friends.length ? Math.max(friends.map((f) => f.id)) + 1 : 1;
  }
...
router.put("/:id", (req, res) => {
  ...
  if (index !== -1) {
    friend[index] = { ...friends[index], ...updatedFriend };
    res.json({
